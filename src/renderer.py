        ...
        for name, widget in widget_list.items():
            ...
            for idx, line in enumerate(to_draw[: actual_height]):
                if y + idx > self.term_sz[0] - 1:
                    break
                self.scr.addnstr(y + idx, x, line[: actual_width], self.term_sz[1] - actual_width)
                # left and right border
                if widget.border:
                    self.scr.addch(y + idx, widget.anchor[0], gen.SHARP_CHS["vertical"])
                    self.scr.addch(y + idx, widget.anchor[0] + widget.width - 1, gen.SHARP_CHS["vertical"])
            ...
            if widget.border:
                self.scr.addnstr(
                    widget.anchor[1],
                    widget.anchor[0],
                    f"{gen.SHARP_CHS["top-left"]}{gen.SHARP_CHS["horizontal"] * actual_width}{gen.SHARP_CHS["top-right"]}",
                    self.term_sz[1] - widget.width,
                )
                self.scr.addnstr(
                    widget.anchor[1] + widget.height - 1,
                    widget.anchor[0],
                    f"{gen.SHARP_CHS["bottom-left"]}{gen.SHARP_CHS["horizontal"] * actual_width}{gen.SHARP_CHS["bottom-right"]}",
                    self.term_sz[1] - widget.width,
                )
